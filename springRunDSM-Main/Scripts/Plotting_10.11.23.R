...
#################
# Annotation
  # This script imports spring-run DSM output data and plots summaries of relevant outputs
  # Users can change the following settings in 'Plotting Settings' to change what is plotted:
    ...
    # Which watersheds for which to summarize individual demographics
      # e.g., sheds = c("Watershed A", "Watershed B", etc.)
#################
...
out.name <- "EIS" # NAA, Alt1-4 (4 phases of Alt2)
...
if(out.name=="EIS"){ # EIS (NAA, Alternatives 1-4)
  n.alt <- 8
...
if(model_type=="Det"){
  ...
  for(i in 1:n.alt){
    adult_df_temp <- input_data[[i]]$out.adult_en_route %>%
      ...
      filter(Watershed%in%sheds) %>%
      mutate(Month=factor(Month, levels=c(3:6)), Alternative=alt_plot_names[i])
  ...
  for(i in 1:n.alt){
    adult_df_temp <- input_data[[i]]$out.prop.nat.stray %>%
      reshape2::melt(varnames=c("Year","Month","Watershed")) %>%
      filter(Watershed%in%sheds) %>%
      mutate(Month=factor(Month, levels=c(3:6)), Alternative=alt_plot_names[i])
  ...
  adult_stray <- ggplot(adult_df, 
                        aes(x=Month, y=value, 
                            fill=factor(Alternative, levels=alt_plot_names)))+
    geom_boxplot() +
    facet_wrap(~Watershed) +
    labs(x="Month", y="Proportion of Adults Straying", fill="Alternative")#+
    #scale_fill_manual(values=col_ref$Hexcode[which(col_ref$Alternative%in%alt_plot_names)]) 
  
  adult_stray_free <- ggplot(adult_df, 
                             aes(x=Month, y=value, 
                                 fill=factor(Alternative, levels=alt_plot_names)))+
    geom_boxplot() +
  ...
  # Summarize sub-yearling in-channel rearing survival (< 42 mm)
  ...
  for(i in 1:n.alt){
    rear_df_temp <- input_data[[i]]$out.river_surv %>%
      ...
      filter(Watershed%in%sheds) %>%
      mutate(Month=factor(Month, levels=c(11:12,1:5)), Alternative=alt_plot_names[i])
  ...
  river_surv_sub <- ggplot(rear_small_df, 
                           aes(x=Month, y=value, 
                               fill=factor(Alternative, levels=alt_plot_names)))+
    geom_boxplot() +
    facet_wrap(~Watershed) +
    labs(x="Month", y="Sub-Yearling Rearing Survival (<42 mm)",
         fill="Alternative")#+
    #scale_fill_manual(values=col_ref$Hexcode[which(col_ref$Alternative%in%alt_plot_names)]) 
  
  river_surv_sub_free <- ggplot(rear_small_df, 
                                aes(x=Month, y=value, 
                                    fill=factor(Alternative, levels=alt_plot_names)))+
    geom_boxplot() +
    facet_wrap(~Watershed, scales="free") +
  ...
  rm(rear_df, rear_df_temp)
  
  # Summarize yearling in-channel rearing survival(< 42 mm)
  ...
  for(i in 1:n.alt){
    rear_df_temp <- input_data[[i]]$out.year_river_surv %>%
      reshape2::melt(varnames=c("Year","Month","Watershed","Size")) %>%
      filter(Watershed%in%sheds) %>%
      mutate(Month=factor(Month, levels=c(5:10)), Alternative=alt_plot_names[i])
  ...
  river_surv_year <- ggplot(rear_small_df, 
                            aes(x=Month, y=value, 
                                fill=factor(Alternative, levels=alt_plot_names)))+
    geom_boxplot() +
    facet_wrap(~Watershed) +
    labs(x="Month", y="Yearling Rearing Survival (<42 mm)", fill="Alternative")#+
    #scale_fill_manual(values=col_ref$Hexcode[which(col_ref$Alternative%in%alt_plot_names)]) 
  
  river_surv_year_free <- ggplot(rear_small_df, 
                                 aes(x=Month, y=value, 
                                     fill=factor(Alternative, levels=alt_plot_names)))+
    geom_boxplot() +
    facet_wrap(~Watershed, scales="free") +
  ...
  # Summarize overall annual proportion of fish that remain to rear and migrate as yearlings
  
  rear_df <- data.frame(Year=numeric(), 
                        value=numeric(), Alternative=character())
  
  for(i in 1:n.alt){
    rear_df_temp <- input_data[[i]]$out.year_prop %>%
      reshape2::melt(varnames=c("Year")) %>%
      mutate(Alternative=alt_plot_names[i])
    
    rear_df <- rbind(rear_df, rear_df_temp)
  }  
  
  year_prop <- ggplot(rear_df, aes(y=value, 
                                   x=factor(Alternative, levels=alt_plot_names)))+
    geom_boxplot() +
    labs(x="Alternative", y="Proportion of Fish Remaining as Yearlings")
  ...
  D_juv_surv <- ggplot(rear_N_small_df, 
                       aes(x=Month, y=value, 
                           fill=factor(Alternative, levels=alt_plot_names)))+
    geom_boxplot() +
    labs(x="Month", y="North Delta Rearing Survival (<42 mm)", fill="Alternative")#+
    #scale_fill_manual(values=col_ref$Hexcode[which(col_ref$Alternative%in%alt_plot_names)]) 
  ...
  rm(migr_df, migr_df_temp, migr_small_df)
...
if(model_type=="Stoch"){
  ...
  river_surv_sub <- ggplot(rear_small_df, 
    ...
    geom_boxplot() +
    facet_wrap(~Watershed) +
    labs(x="Month", y="Sub-yearling Rearing Survival (<42 mm)", fill="Alternative")#+
    #scale_fill_manual(values=col_ref$Hexcode[which(col_ref$Alternative%in%alt_plot_names)]) 
  ...
  rm(rear_df_temp, rear_df, rear_small_df)

  # Summarize overall annual proportion of fish that remain to rear and migrate as yearlings
  
  rear_df <- data.frame(Year=numeric(), Iteration=numeric(),
                        value=numeric(), Alternative=character())
  
  for(i in 1:n.alt){
    rear_df_temp <- input_data[[i]]$out.year_prop %>%
      reshape2::melt(varnames=c("Year","Iteration")) %>%
      mutate(Alternative=alt_plot_names[i])
  ...
  ###################################
  # Diagnostic plots - Butte Creek
  ###################################

diag_sheds <- c("Butte Creek")
...
if(model_type=="Stoch"){
  ...
  for(i in 1:n.alt){
    rear_df_temp <- input_data[[i]]$out.river_surv %>%
      reshape2::melt(varnames=c("Year","Month","Watershed","Size","Iteration")) %>%
      filter(Watershed%in%diag_sheds) %>%
      mutate(Month=factor(Month, levels=c(11:12,1:5)), Alternative=alt_plot_names[i])

    rear_df <- rbind(rear_df, rear_df_temp)
  }
  
  river_surv_diag <- ggplot(rear_df, aes(x=Month, y=value, fill=Size))+
    geom_boxplot() +
    facet_wrap(~factor(Alternative, levels=alt_plot_names)) +
    labs(x="Month", y="Sub-yearling Rearing Survival")
  
  river_surv_diag
  
  ggsave(paste0("./springRunDSM-Main/Output/Figures/",out.name,"_",model_type,"_Juv Rear Diag Stoch_Butte.png"),
         river_surv_diag, width=6, height=5, units=c("in"))
  ...
  adult_stray_diag <- ggplot(adult_df, 
                             aes(x=Month, y=value, 
                                 fill=factor(Alternative, levels=alt_plot_names)))+
    geom_boxplot() +
    facet_wrap(~Watershed) +
  ...
  ggsave(paste0("./springRunDSM-Main/Output/Figures/",out.name,"_",model_type,"_Adult Stray Diag Stoch_Butte.png"),
         adult_stray_diag, width=6, height=5, units=c("in"))
...
if(model_type=="Det"){
  ...
  ggsave(paste0("./springRunDSM-Main/Output/Figures/",out.name,"_",model_type,"_Juv Rear Diag Det_Butte.png"),
         river_surv_diag, width=6, height=5, units=c("in"))
  ...
  ggsave(paste0("./springRunDSM-Main/Output/Figures/",out.name,"_",model_type,"_Adult Stray Diag Det_Butte.png"),
         adult_stray_diag, width=6, height=5, units=c("in"))
...
##################################
  ...
  # Lambda calculations
    ...
    # Take sum across all watersheds - total spring-run lambda
  #####################
...
if(model_type=="Det"){
  ...
  for(j in 1:n.alt){
    for(i in 2:20){
      lambda[j,i-1] <- sum(input_data[[j]]$out.all_spawners[,i])/sum(input_data[[j]]$out.all_spawners[,i-1])
    }
    lambda_mean[j] <- exp(mean(log(lambda[j,])))
    end_lambda[j] <- sum(input_data[[j]]$out.all_spawners[,20])/sum(input_data[[j]]$out.all_spawners[,1])
...
if(model_type=="Stoch"){
  ...
  for(k in 1:n.alt){
    for(j in 1:input_data[[1]]$n.iter){
      for(i in 2:20){
        lambda[k,i-1,j] <- sum(input_data[[k]]$out.all_spawners[,i,j])/sum(input_data[[k]]$out.all_spawners[,i-1,j])
      }
      lambda_mean[k,j] <- exp(mean(log(lambda[k,,j])))
      end_lambda[k,j] <- sum(input_data[[k]]$out.all_spawners[,20,j])/sum(input_data[[k]]$out.all_spawners[,1,j])
    }
...
if(model_type=="Det"){
  ...
  # All spawners - Upper Sac, Clear Creek
  ...
  all_spawners <- ggplot(spawn_df, 
                         aes(x=Year, y=`All Spawners`, 
                             col=factor(Alternative, levels=alt_plot_names)))+
    geom_line() +
    facet_wrap(~Watershed)+
  ...
  # All spawners, aggregated across watersheds
  ...
  spawn_df <- spawn_df %>% group_by(Year, Alternative) %>%
    summarize(`All Spawners (Across Watersheds)`=sum(`All Spawners`))
  
  all_spawners_aggr <- ggplot(spawn_df, 
                              aes(x=Year, y=`All Spawners (Across Watersheds)`, 
                                  col=factor(Alternative, levels=alt_plot_names)))+
    geom_line()+
    labs(col="Alternative")#+
    #scale_colour_manual(values=col_ref$Hexcode[which(col_ref$Alternative%in%alt_plot_names)]) 
  
  # Nat spawners - Upper Sac, Clear Creek
  ...
  nat_spawners <- ggplot(spawn_df, 
                         aes(x=Year, y=`Natural Spawners`, 
                             col=factor(Alternative, levels=alt_plot_names)))+
    geom_line()+
    facet_wrap(~Watershed)+
    labs(col="Alternative")#+
  ...
  # Nat spawners, aggregated across watersheds
  ...
  spawn_df <- spawn_df %>% group_by(Year, Alternative) %>%
    summarize(`Natural Spawners (Across Watersheds)`=sum(`Natural Spawners`))
  
  nat_spawners_aggr <- ggplot(spawn_df, 
                              aes(x=Year, y=`Natural Spawners (Across Watersheds)`, 
                                  col=factor(Alternative, levels=alt_plot_names)))+
    geom_line()+
    labs(col="Alternative")#+
    #scale_colour_manual(values=col_ref$Hexcode[which(col_ref$Alternative%in%alt_plot_names)]) 
  ...
  # Natural spawners, relative to some baseline
    ...
    # Only do this for all natural spawners, across watersheds
  
  spawn_df_ref <- input_data[[1]]$out.nat_spawners %>%
    reshape2::melt(varnames=c("Watershed","Year")) %>%
    mutate(Scenario=alt_plot_names[1], `Natural Spawners`=value) %>%
    group_by(Year) %>%
    summarise(`Natural Spawners Sum`=sum(`Natural Spawners`),
              Mean=mean(`Natural Spawners Sum`), 
              Lo_95=quantile(`Natural Spawners Sum`,0.025),
              Hi_95=quantile(`Natural Spawners Sum`,0.975))
  ...
  nat_spawners_diff <- ggplot(spawn_df, 
                              aes(x=Year, y=`% Difference (Mean)`, 
                                  col=factor(Alternative, levels=alt_plot_names)))+
    geom_line()+
    labs(y="% Difference in Natural Abundance",
         col="Alternative")#+
    #scale_colour_manual(values=col_ref$Hexcode[which(col_ref$Alternative%in%alt_plot_names)])  
  
  # Lambda plot
  
  lambda_all <- ggplot(lambda_plot, 
                       aes(x=Year, y=Lambda, 
                           col=factor(Alternative, levels=alt_plot_names)))+
    geom_line()+
  ...
  lambda_all_diff <- ggplot(lambda_diff_plot, 
                            aes(x=Year, y=Lambda_diff, 
                                col=factor(Alternative, levels=alt_plot_names)))+
    geom_line()+
    labs(y="% Difference in Lambda", col="Alternative")#+
    #scale_colour_manual(values=col_ref$Hexcode[which(col_ref$Alternative%in%alt_plot_names)])
}
...
if(model_type=="Stoch"){
  ...
  all_spawners <- ggplot(spawn_df, aes(x=Year, y=`All Spawners`))+
    ...
    facet_wrap(Watershed~factor(Alternative, levels=alt_plot_names))
  
  all_spawners_free <- ggplot(spawn_df, aes(x=Year, y=`All Spawners`))+
    geom_line(col="black", aes(group=Iteration))+
    geom_smooth(spawn_df, mapping=aes(x=Year, y=`All Spawners`), method="gam")+  
    facet_wrap(Watershed~factor(Alternative, levels=alt_plot_names), scales="free")
  ...
  spawn_df <- spawn_df %>% group_by(Year, Iteration, Alternative) %>%
    summarize(`All Spawners (Across Watersheds)`=sum(`All Spawners`))
  
  all_spawners_aggr <- ggplot(spawn_df, aes(x=Year, y=`All Spawners (Across Watersheds)`))+
    geom_line(col="black", aes(group=Iteration))+
    geom_smooth(spawn_df, mapping=aes(x=Year, y=`All Spawners (Across Watersheds)`), method="gam")+  
    facet_wrap(~factor(Alternative, levels=alt_plot_names))
  ...
  nat_spawners_free <- ggplot(spawn_df, aes(x=Year, y=`Natural Spawners`))+
    geom_line(col="black", aes(group=Iteration))+
    geom_smooth(spawn_df, mapping=aes(x=Year, y=`Natural Spawners`), method="gam")+ # how do 
    facet_wrap(Watershed~factor(Alternative, levels=alt_plot_names), scales="free")
  
  # Natural spawners, aggregated across watersheds
  ...
  nat_spawners_aggr <- ggplot(spawn_df, aes(x=Year, y=`Natural Spawners (Across Watersheds)`))+
    geom_line(col="black", aes(group=Iteration))+
    geom_smooth(spawn_df, mapping=aes(x=Year, y=`Natural Spawners (Across Watersheds)`), method="gam")+  
    facet_wrap(~factor(Alternative, levels=alt_plot_names))
  
  # Iteration-specific lambda values (boxplots by Scenario)
    # Lambda summarized population growth across ESU
  
  lambda_mean <- ggplot(lambda_df, 
                        aes(x=factor(Alternative, levels=alt_plot_names), 
                            y=Lambda_mean))+
    geom_boxplot()+
...
if(model_type=="Stoch"){
  ...
  all_spawners_diag <- ggplot(spawn_df, aes(x=Year, y=`All Spawners`))+
    geom_line(col="black", aes(group=Iteration))+
    geom_smooth(spawn_df, mapping=aes(x=Year, y=`All Spawners`), method="gam")+  
    facet_wrap(Watershed~factor(Alternative, levels=alt_plot_names))
  
  ggsave(paste0("./springRunDSM-Main/Output/Figures/",out.name,"_",model_type,"_All Spawner Diag Stoch_Butte.png"),
         all_spawners_diag, width=6, height=5, units=c("in"))
...
if(model_type=="Det"){
  ...
  all_spawners_diag <- ggplot(spawn_df, 
                              aes(x=Year, y=`All Spawners`, 
                                  col=factor(Alternative, levels=alt_plot_names)))+
    geom_line() +
    facet_wrap(~Watershed)+
  ...
  ggsave(paste0("./springRunDSM-Main/Output/Figures/",out.name,"_",model_type,"_All Spawners Diag Det_Butte.png"),
         all_spawners_diag, width=6, height=5, units=c("in"))
...
ggsave(paste0("./springRunDSM-Main/Output/Figures/",out.name,"_",model_type,"_All Spawners.png"),
       all_spawners, width=6, height=6, units=c("in"))
ggsave(paste0("./springRunDSM-Main/Output/Figures/",out.name,"_",model_type,"_All Spawners Aggr.png"),
       all_spawners_aggr, width=6, height=6, units=c("in"))
ggsave(paste0("./springRunDSM-Main/Output/Figures/",out.name,"_",model_type,"_Natural Spawners.png"),
       nat_spawners, width=6, height=6, units=c("in"))
ggsave(paste0("./springRunDSM-Main/Output/Figures/",out.name,"_",model_type,"_Natural Spawners Aggr.png"),
       nat_spawners_aggr, width=6, height=6, units=c("in"))
ggsave(paste0("./springRunDSM-Main/Output/Figures/",out.name,"_",model_type,"_Lambda Timeseries.png"),
       lambda_all, width=6, height=6, units=c("in"))

if(model_type=="Det"){
  ggsave(paste0("./springRunDSM-Main/Output/Figures/",out.name,"_",model_type,"_Natural Spawners Perc Diff.png"),
         nat_spawners_diff, width=6, height=4, units=c("in"))
  ggsave(paste0("./springRunDSM-Main/Output/Figures/",out.name,"_",model_type,"_Lambda Timeseries Perc Diff.png"),
         lambda_all_diff, width=6, height=4, units=c("in"))
}
if(model_type=="Stoch"){
  ggsave(paste0("./springRunDSM-Main/Output/Figures/",out.name,"_",model_type,"_Lambda Mean Boxplot.png"),
         lambda_mean, width=6, height=4, units=c("in"))
  ggsave(paste0("./springRunDSM-Main/Output/Figures/",out.name,"_",model_type,"_Lambda End Boxplot.png"),
         lambda_end, width=6, height=4, units=c("in"))
  ggsave(paste0("./springRunDSM-Main/Output/Figures/",out.name,"_",model_type,"_Lambda WYT Boxplot.png"),
         lambda_mean_wyt, width=6, height=4, units=c("in"))
}

# Life state transition parameters, not grouped by month + WYT
ggsave(paste0("./springRunDSM-Main/Output/Figures/",out.name,"_",model_type,"_Adult En Route.png"),
       adult_en_route, width=6, height=5, units=c("in"))
ggsave(paste0("./springRunDSM-Main/Output/Figures/",out.name,"_",model_type,"_Adult Straying.png"),
       adult_stray, width=6, height=5, units=c("in"))
ggsave(paste0("./springRunDSM-Main/Output/Figures/",out.name,"_",model_type,"_Adult Prespawn.png"),
       pre.spawn.S, width=6, height=5, units=c("in"))
ggsave(paste0("./springRunDSM-Main/Output/Figures/",out.name,"_",model_type,"_Juv Rear Sub S.png"),
       river_surv_sub, width=6, height=5, units=c("in"))
ggsave(paste0("./springRunDSM-Main/Output/Figures/",out.name,"_",model_type,"_Juv Rear Sub S Free.png"),
       river_surv_sub_free, width=6, height=5, units=c("in"))
ggsave(paste0("./springRunDSM-Main/Output/Figures/",out.name,"_",model_type,"_Juv Rear Yearl S.png"),
       river_surv_year, width=6, height=5, units=c("in"))
ggsave(paste0("./springRunDSM-Main/Output/Figures/",out.name,"_",model_type,"_Yearling Prop.png"),
       year_prop, width=6, height=5, units=c("in"))
ggsave(paste0("./springRunDSM-Main/Output/Figures/",out.name,"_",model_type,"_Juv N Delta Rear S.png"),
       D_juv_surv, width=6, height=5, units=c("in"))
ggsave(paste0("./springRunDSM-Main/Output/Figures/",out.name,"_",model_type,"_Prop Pulse Leave S.png"),
       p.pulse.leave, width=6, height=5, units=c("in"))
ggsave(paste0("./springRunDSM-Main/Output/Figures/",out.name,"_",model_type,"_Prop Pulse Leave S.png"),
       p.pulse.leave, width=6, height=5, units=c("in"))
ggsave(paste0("./springRunDSM-Main/Output/Figures/",out.name,"_",model_type,"_Juv UM Sac Migr VL.png"),
       UM.Sac.S, width=6, height=5, units=c("in"))
ggsave(paste0("./springRunDSM-Main/Output/Figures/",out.name,"_",model_type,"_S Delta Entrainment.png"),
       prop.dlt.entrain, width=6, height=5, units=c("in"))
ggsave(paste0("./springRunDSM-Main/Output/Figures/",out.name,"_",model_type,"_Juv N Delta Migr VL.png"),
       Sac.Delt.S, width=6, height=5, units=c("in"))
ggsave(paste0("./springRunDSM-Main/Output/Figures/",out.name,"_",model_type,"_Juv S Delta Migr VL.png"),
       newDsurv, width=6, height=5, units=c("in"))

# Migratory survival parameters, vl size, grouped by month + WYT

ggsave(paste0("./springRunDSM-Main/Output/Figures/",out.name,"_",model_type,"_Juv UM Sac Migr VL Facet.png"),
       UM.Sac.S.facet, width=7, height=9, units=c("in"))
ggsave(paste0("./springRunDSM-Main/Output/Figures/",out.name,"_",model_type,"_Juv LM Sac Migr VL Facet.png"),
       LM.Sac.S.facet, width=7, height=9, units=c("in"))
ggsave(paste0("./springRunDSM-Main/Output/Figures/",out.name,"_",model_type,"_Juv LL Sac Migr VL Facet.png"),
       LL.Sac.S.facet, width=7, height=9, units=c("in"))
ggsave(paste0("./springRunDSM-Main/Output/Figures/",out.name,"_",model_type,"_Juv N Delta Migr VL Facet.png"),
       Sac.Delt.S.facet, width=7, height=9, units=c("in"))
ggsave(paste0("./springRunDSM-Main/Output/Figures/",out.name,"_",model_type,"_Juv S Delta Migr VL Facet.png"),

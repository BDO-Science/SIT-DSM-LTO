...
```{r tab.cap="Table EIS.1b. Predicted percent difference in annual total spring-run spawner abundance in the Central Valley, including both natural- and hatchery-origin fish, relative to NAA.", tab.id="TableEISb1", echo=FALSE}
table1b <- read.csv(here(paste0("springRunDSM-Main/Output/Tables/EIS_AllSpawnersPercDiff.csv")))
colnames(table1b) <- gsub("."," ", colnames(table1b), fixed=TRUE)
table1b <- round(table1b,3)
table1b
```
...
```{r tab.cap="Table EIS.2b. Predicted percent difference in annual natural-origin spring-run spawner abundance in the Central Valley, relative to NAA.", tab.id="TableEIS2b", echo=FALSE}
table2b <- read.csv(here(paste0("springRunDSM-Main/Output/Tables/EIS_NatSpawnersPercDiff.csv")))
colnames(table2b) <- gsub("."," ", colnames(table2b), fixed=TRUE)
table2b <- round(table2b,3)
table2b
```
...
```{r tab.cap="Table EIS.3b. Predicted percent difference in mean lambda (Nt+1/Nt) for total spring-run spawner abundance in the Central Valley, including both natural- and hatchery-origin fish, relative to NAA.", tab.id="TableEIS3b", echo=FALSE}
table3b <- read.csv(here(paste0("springRunDSM-Main/Output/Tables/EIS_MeanLambdaPercDiff.csv")))
table3b$WYT <- "All"

mlambda_wyt <- read.csv(here(paste0("springRunDSM-Main/Output/Tables/EIS_WYTMeanLambdaPercDiff.csv")))
table3b <- rbind(mlambda_wyt, table3b)

colnames(table3b) <- gsub("."," ", colnames(table3b), fixed=TRUE)
table3b[,2:length(table3b)] <- round(table3b[,2:length(table3b)], 3)
table3b
```
...
```{r tab.cap="Table EIS.4b. Predicted percent difference in end lambda (Nt=19/Nt=1) for total spring-run spawner abundance in the Central Valley, including both natural- and hatchery-origin fish.", tab.id="TableEIS4b", echo=FALSE}
table4b <- read.csv(here(paste0("springRunDSM-Main/Output/Tables/EIS_EndLambdaPercDiff.csv")))
colnames(table4b) <- gsub("."," ", colnames(table4b), fixed=TRUE)
table4b <- round(table4b, 3)
table4b
```

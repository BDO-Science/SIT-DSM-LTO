...
#########################################
# Annotation
  # This wrapper script is based on SpringRunStochasticDSM_11_13_2019.R
    # Located in the springRunDSM-Main repository, pulled from https://github.com/CVPIA-OSC/winterRunDSM/tree/main
  ...
  # Changes to the run_scenarios() function
    # I moved the run_scenarios() function (renamed life_cycle_model(), life_cycle_model_v2()) into its own script
      # (located in 'functions' sub-folder) and call it in this script
    # I added a new input, 'alt', that tells the model which inputs to use
...
# Life cycle model function
  # This is actually made redundant by the for() loop a few lines below
#source(here("springRunDSM-Main","functions","Life cycle model_spring.R"))
#source(here("springRunDSM-Main","functions","Life cycle model_spring_v2.R"))

# Dependent functions of the life cycle model function
fun.place<-paste(getwd(),"/springRunDSM-main/functions",sep="")
...
# Load Model Inputs function
  # This is also made redundant by the for() loop a couple lines above
#source(here("springRunDSM-Main","functions","Load Model Inputs.R"))

####################
# Deterministic Runs
####################

scenarioRun = 0 # DON'T CHANGE
  ...
  # Only use this for generating seeds for life_cycle_model_v2 (requires going into script manually)
    # Need to do the following to obtain the spawners_seed.rds object: 
      # Set the following:
        ...
        #pctil=1
        #alt="originalDSM"
      # Go into the script for life_cycle_model and manually change number of yr2 (~line 165) from 1:25 to 1:5
      # Run model manually, in "Life cycle model.R", from ~lines 21-1854 (end of year loop)
      # Save spwners object as follows: saveRDS(spwners, "./springRunDSM-Main/spawners_seed_recalibration.rds")

# Run updated model

  # Set seed spanwers - see above
#seed <- readRDS("./springRunDSM-Main/spawners_seed.rds")
seed <- readRDS("./springRunDSM-Main/spawners_seed_recalibration.rds")
...
# Save results
name<-paste0(here("springRunDSM-main", "Output", "SR_Recal_Det_Scen"),scenarioRun,"_Alt_",alt,"_v2")
...
out.adult_en_route <- out.prop.nat.stray <- array(NA, dim=c(20,4,31,n.iter),
                            dimnames=list(1980:1999,3:6,watershed.names,1:n.iter))
out.pre.spawn.S <- array(NA, dim=c(20,2,31,n.iter),
                         dimnames=list(1980:1999,c("Summer","Fall"),watershed.names,1:n.iter))
out.p.pulse.leave <- out.river_surv <- out.flood_surv <- array(NA, dim=c(20,7,31,4,n.iter),
                                                               dimnames=list(1980:1999,c(11:12,1:5),watershed.names,size.names,1:n.iter))
out.year_river_surv <- out.year_flood_surv <- array(NA, dim=c(20,6,31,4,n.iter),
                                                    dimnames=list(1980:1999,c(5:10),watershed.names,size.names,1:n.iter))
out.year_prop <- array(NA, dim=c(20,n.iter), dimnames=list(1980:1999,1:n.iter))
out.D_juv_surv <- out.Sac.Delt.S <- array(NA, dim=c(20,7,2,4,n.iter),
                                          dimnames=list(1980:1999,c(11:12,1:5),c("N","S"),size.names,1:n.iter))
out.UM.Sac.S <- out.LM.Sac.S <- out.LL.Sac.S <- array(NA, dim=c(20,7,4,n.iter),
                                                      dimnames=list(1980:1999,c(11:12,1:5),size.names,1:n.iter))
out.newDsurv <- array(NA, dim=c(20,7,4,4,n.iter),
                      dimnames=list(1980:1999,c(11:12,1:5),c("N","O1","O2","O3"),size.names,1:n.iter))
out.prop.dlt.entrain <- array(NA, dim=c(20,7,n.iter),
                              dimnames=list(1980:1999,c(11:12,1:5),1:n.iter))
...
for(iter in 1:n.iter){
  ...
  #base<-life_cycle_model_spring(stochastic=1,scenario=scenarioRun,vary="Nothing",pctil=1, alt=alt)
  ...
  out.prop.nat.stray[,,,iter] <- base$out.prop.nat.stray
  ...
  out.year_river_surv[,,,,iter] <- base$out.year_river_surv
  out.year_flood_surv[,,,,iter] <- base$out.year_flood_surv
  out.year_prop[,iter] <- base$out.year_prop
...
stoch_results <- list(out.all_spawners=out.all_spawners, 
                      ...
                      out.prop.nat.stray=out.prop.nat.stray,
                      ...
                      out.year_river_surv=out.year_river_surv,
                      out.year_flood_surv=out.year_flood_surv,
                      out.year_prop=out.year_prop,
...
# Save results
name<-paste0(here("springRunDSM-main", "Output", "SR_Recal_Stoch_Scen"),scenarioRun,"_Alt_",alt,"_Iter",n.iter,"_v2")

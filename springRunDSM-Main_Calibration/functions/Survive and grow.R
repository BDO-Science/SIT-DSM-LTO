...
####################################
# Script annotation for LTO Modeling
  # Modified code for application of rearing survival
    # Now, size-class specific rearing survival estimates are applied correctly
      # to the matrix of juvenile abundances (watershed x size class), no matter
      # whether rearing survivals are expressed as a vector or matrix
  # All other code and annotation is left unchanged
####################################
...
rearfunc<-function(river_rear,flood_rear,trans_mat_river,trans_mat_flood,flood_surv,river_surv,stoch){
  ...
  if(is.vector(river_surv)){ # multiply matrix by a vector correctly
    ...
    } else{junk<-round(t(t(river_rear)*river_surv))}
  } else { # multiply matrix by a matrix
    if(max(river_rear)<=1000000000 & stoch==1){
      junk<-matrix(rbinom(spaces,prob=as.vector(t(river_surv)),size=round(as.vector(t(river_rear)))),ncol=4,byrow=TRUE)
    } else{junk<-round(river_rear*river_surv)} 
    #} else{junk<-round(t(t(river_rear)*river_surv))}   
  ...
  if(is.vector(flood_surv)){
    ...
    } else{junk2<-round(t(t(flood_rear)*flood_surv))}
  } else {
    if(max(flood_rear)<=1000000000 & stoch==1){
      junk2<-matrix(rbinom(spaces,prob=as.vector(t(flood_surv)),size=round(as.vector(t(flood_rear)))),ncol=4,byrow=TRUE)
    } else{junk2<-round(flood_rear*flood_surv)}
  ...
  #survival
  #river_rear<-(junk*stoch)+(river_rear)*river_surv)*(1-stoch) # OLD, INCORRECT
  #flood_rear<-(junk2*stoch)+(flood_rear*flood_surv)*(1-stoch) # OLD, INCORRECT
  river_rear<-junk
  flood_rear<-junk2

#################
# Annotation 
  # This script imports winter-run DSM output data and provides summaries
...
juv.at.chips_df <- data.frame(Year=numeric(),
...
for(i in 1:n.alt){
  ...
  # Plotting
juv.at.chips <- ggplot(juv.at.chips_df, aes(x=Year, y=Abundance, col=Alternative))+
  geom_line() +
  labs(y="Juvenile Abundance passing Chipps Island", x="Brood Year")
juv.at.chips
ggsave(paste0("./winterRunDSM-Main/Output/Figures/",out.name,"_SRKW_Juv Abund at Chipps.png"),
       width=6, height=5, units=c("in"))

  # Base table
juv.at.chips_table <- juv.at.chips_df %>%
  pivot_wider(names_from=Alternative, values_from=Abundance)
write.csv(juv.at.chips_table, paste0("./winterRunDSM-Main/Output/Tables/",out.name,"_SRKW_WR Juv Abund at Chipps.csv"),
          row.names=FALSE)
...
write.csv(juv.at.chips_tableb, paste0("./winterRunDSM-Main/Output/Tables/",out.name,"_SRKW_WR Juv Abund at Chipps Perc Diff.csv"),
          row.names=FALSE)
...
# Total juvenile biomass at Chipps, across size classes
  ...
  # Creating summary object
bio.at.chips_df <- data.frame(Year=numeric(),
                              Alternative=character(),
                              Biomass=numeric())
...
for(i in 1:n.alt){
  ...
  # Plotting
bio.at.chips <- ggplot(bio.at.chips_df, aes(x=Year, y=Biomass/1000, col=Alternative))+
  geom_line() +
  labs(y="Juvenile Biomass passing Chipps Island (kg)", x="Brood Year")
bio.at.chips
ggsave(paste0("./winterRunDSM-Main/Output/Figures/",out.name,"_SRKW_Juv Biomass at Chipps.png"),
       width=6, height=5, units=c("in"))

  # Base table
bio.at.chips_table <- bio.at.chips_df %>%
  pivot_wider(names_from=Alternative, values_from=Biomass)
write.csv(bio.at.chips_table, paste0("./winterRunDSM-Main/Output/Tables/",out.name,"_SRKW_WR Juv Biomass at Chipps.csv"),
          row.names=FALSE)
...
bio.at.chips_tableb <- bio.at.chips_table %>%
  mutate(Alt1=(Alt1-NAA)/NAA*100,
         ...
         Alt3=(Alt3-NAA)/NAA*100,
         Alt4=(Alt4-NAA)/NAA*100) %>%
  select(Year,Alt1,`Alt2wTUCPwoVA`,`Alt2woTUCPwoVA`,`Alt2woTUCPDeltaVA`,`Alt2woTUCPAllVA`,
         Alt3, Alt4)
write.csv(bio.at.chips_tableb, paste0("./winterRunDSM-Main/Output/Tables/",out.name,"_SRKW_WR Juv Biomass at Chipps Perc Diff.csv"),
          row.names=FALSE)
...
# Total juvenile abundance at Golden Gate, across size classes
  ...
  # Creating summary object
juv.at.GG_df <- data.frame(Year=numeric(),
                           Alternative=character(),
                           Abundance=numeric())
...
for(i in 1:n.alt){
  ...
  # Plotting
juv.at.GG <- ggplot(juv.at.GG_df, aes(x=Year, y=Abundance, col=Alternative))+
  geom_line() +
  labs(y="Juvenile Abundance passing Golden Gate", x="Brood Year")
juv.at.GG
ggsave(paste0("./winterRunDSM-Main/Output/Figures/",out.name,"_SRKW_Juv Abund at GG.png"),
       width=6, height=5, units=c("in"))

  # Base table
juv.at.GG_table <- juv.at.GG_df %>%
  pivot_wider(names_from=Alternative, values_from=Abundance)
write.csv(juv.at.GG_table, paste0("./winterRunDSM-Main/Output/Tables/",out.name,"_SRKW_WR Juv Abund at GG.csv"),
          row.names=FALSE)
...
juv.at.GG_tableb <- juv.at.GG_table %>%
  ...
  select(Year,Alt1,`Alt2wTUCPwoVA`,`Alt2woTUCPwoVA`,`Alt2woTUCPDeltaVA`,`Alt2woTUCPAllVA`,
         Alt3,Alt4)
write.csv(juv.at.GG_tableb, paste0("./winterRunDSM-Main/Output/Tables/",out.name,"_SRKW_WR Juv Abund at GG Perc Diff.csv"),
          row.names=FALSE)
...
# Total juvenile biomass at Golden Gate, across size classes
  ...
  # Creating summary object
bio.at.GG_df <- data.frame(Year=numeric(),
                           Alternative=character(),
                           Biomass=numeric())
...
for(i in 1:n.alt){
  ...
  # Plotting
bio.at.GG <- ggplot(bio.at.GG_df, aes(x=Year, y=Biomass/1000, col=Alternative))+
  geom_line() +
  labs(y="Juvenile Biomass passing Golden Gate (kg)", x="Brood Year")
bio.at.GG
ggsave(paste0("./winterRunDSM-Main/Output/Figures/",out.name,"_SRKW_Juv Biomass at GG.png"),
       width=6, height=5, units=c("in"))

  # Base table
bio.at.GG_table <- bio.at.GG_df %>%
  pivot_wider(names_from=Alternative, values_from=Biomass)
write.csv(bio.at.GG_table, paste0("./winterRunDSM-Main/Output/Tables/",out.name,"_SRKW_WR Juv Biomass at GG.csv"),
          row.names=FALSE)
...
bio.at.GG_tableb <- bio.at.GG_table %>%
  ...
  select(Year,Alt1,`Alt2wTUCPwoVA`,`Alt2woTUCPwoVA`,`Alt2woTUCPDeltaVA`,`Alt2woTUCPAllVA`,
         Alt3,Alt4)
write.csv(bio.at.GG_tableb, paste0("./winterRunDSM-Main/Output/Tables/",out.name,"_SRKW_WR Juv Biomass at GG Perc Diff.csv"),
          row.names=FALSE)

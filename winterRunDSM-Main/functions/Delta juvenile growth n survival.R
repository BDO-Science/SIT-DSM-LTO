...
# Script annotation for LTO Modeling
  # Modified code for application of rearing survival
    # Now, size-class specific rearing survival estimates are applied correctly
      # to the matrix of juvenile abundances (watershed x size class), when
      # rearing survival vector is applied to the juvenile abundance matrix
...
Delt.rearfunc<-function(delt_juv,trans_mat_river,D_juv_surv,stoch){
  ...
  if(max(delt_juv)<=1000000000 & stoch==1){
    ...
    #} else{junk<-round(delt_juv*D_juv_surv)} # OLD, INCORRECT
  } else{junk<-round(t(t(delt_juv)*D_juv_surv))}
